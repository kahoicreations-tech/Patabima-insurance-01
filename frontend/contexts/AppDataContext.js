...
export const AppDataProvider = ({ children }) => {
  ...
  const fetchUser = useCallback(async (force = false) => {
    console.log('[AppDataContext] fetchUser called - force:', force, 'isFresh:', isFresh('user'), 'user exists:', !!user);
    if (!force && isFresh('user') && user) {
      console.log('[AppDataContext] Returning cached user');
      return user;
    }
    try {
      console.log('[AppDataContext] Fetching user from API...');
      const data = await usersAPI.getCurrentUser();
      console.log('[AppDataContext] User fetched successfully:', data?.full_names || 'No name', 'agent_code:', data?.agent_code);
      setUser(data);
      markFresh('user');
      console.log('[AppDataContext] User state updated');
      return data;
    } catch (e) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Clear loading state immediately after user fetch completes
      // This ensures the UI can render with user data right away
      console.log('[AppDataContext] User data ready, clearing loading state');
      setLoading(false);
      
      // Schedule deferred data fetch for non-critical data
