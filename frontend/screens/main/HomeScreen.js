...
export default function HomeScreen() {
  ...
  // Get data from AppDataContext instead of fetching again
  const { 
    user: contextUser, 
    renewals, 
    extensions, 
  ...
  // Transform context user to agentData format
  const agentData = useMemo(() => {
    console.log('[HomeScreen] agentData useMemo - contextUser:', contextUser?.full_names || 'null');
    if (!contextUser) return null;
    
    // Don't return agentData if critical fields are missing
    if (!contextUser.full_names && !contextUser.agent_code) {
      console.warn('[HomeScreen] contextUser exists but missing critical fields');
      return null;
    }
    
    return {
      agentCode: contextUser.agent_code || 'N/A',
      fullName: contextUser.full_names || 'User',
      ...
      nextPayout: contextUser.next_commission_date || 'Not Available',
      phoneNumber: contextUser.phonenumber || '', 
      email: contextUser.email || '',
      role: contextUser.role || 'AGENT',
      lastLogin: contextUser.last_login || null
    };
  }, [contextUser]);
  
  // Use data from AppDataContext
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    try {
      // Refresh data from AppDataContext
      const promises = [
        fetchUser(true),
        fetchRenewals(true),
  ...
  console.log('[HomeScreen] Loading gate - dataLoading:', dataLoading, 'agentData:', !!agentData, 'contextUser:', !!contextUser);
  if (dataLoading || !agentData) {
    return (
